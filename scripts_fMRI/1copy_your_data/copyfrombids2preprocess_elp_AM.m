...
addpath(genpath('/panfs/accrepfs.vampire/data/booth_lab/AM/ELP_autism/scripts/1copy_your_data')); %make sure you copied the code expand_path.m in your codes
global CCN;

root='/panfs/accrepfs.vampire/data/booth_lab/datasets/12_ds003604/bids';  %This is the path where the data raw data sits
subjects={'sub-5022'}; % you can either manually put in your subjects (e.g.'sub-5004' 'sub-5009') or leave it empty and define a path of an excel that contains subject numbers as indicated below. 
data_info='/panfs/accrepfs.vampire/data/booth_lab/AM/ELP_autism/Subjects.xlsx'; %In this excel, there should be a column of subjects with the header (subjects). The subjects should all be sub plus numbers (sub-5002).
if isempty(subjects)
    M=readtable(data_info);
    subjects=M.subjects;
end

new_root='/panfs/accrepfs.vampire/data/booth_lab/AM/ELP_autism/data'; %This is the folder path where you want to do analysis
CCN.funcf1='sub*Phon*bold.nii.gz'; %This is the functional folder you want to copy. In this example, it's just Plaus and Gram.
CCN.funcf2='sub*Phon*bold.nii.gz';
...
writefile='multiple_T1w_subjects_bids_ses-7_4.txt'; % This file is used to record repetited T1s, and will be used in code delete_bad_t1.m later when you want unique t1 to preprocess the data. 
