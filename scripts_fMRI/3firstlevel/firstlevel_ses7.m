...
%parpool;
addpath(genpath('/panfs/accrepfs.vampire/data/booth_lab/AM/ELP_autism/scripts/3firstlevel')); % the path of your scripts
...
analysis_folder='ses7_analysis_phon'; % the name of your first level modeling folder different for ses-7 and ses-9

% define your data path
data=struct();
root='/panfs/accrepfs.vampire/data/booth_lab/AM/ELP_autism/';  %your project path
model_deweight='deweight'; % the deweigthed modeling folder, it will be inside of your analysis folder

global CCN
CCN.preprocessed='data'; % your data folder
CCN.session='ses-7'; % the time points you want to analyze
CCN.func_pattern='sub*Phon*_bold'; % the name of your functional folders
CCN.file='vs6_wsub*_*bold.nii'; % the name of your preprocessed data (4d)%
...
bids_folder='/panfs/accrepfs.vampire/data/booth_lab/datasets/12_ds003604/bids'; % if you assign 0 to events_file_exist, then you mask fill in this path, so it can read events.tsv file for individual onsets from bids folder
...
%conditions{1}={'S_C' 'S_H' 'S_L' 'S_U'};
%conditions{2}={'S_C' 'S_H' 'S_L' 'S_U'};
...
%contrasts={'weakstrong_vs_perceptual'};%{'onsetrhyme_vs_perceptual' ...
...
rp_w=zeros(1,6);
%empty=zeros(1,10);
%weights={[weakstrong_vs_perceptual rp_w weakstrong_vs_perceptual rp_w]};
weights={[onsetrhyme_vs_perceptual rp_w onsetrhyme_vs_perceptual rp_w]};
